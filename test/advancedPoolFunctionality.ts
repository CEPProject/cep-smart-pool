...
describe("Advanced Pool Functionality", function () {
  ...
  let startBlock: number; // 开始区块
  let endBlock: number; // 结束区块
  ...
  beforeEach(async () => {
    ...
    for (let i = 0; i < 8; i++) { 
      ...
      // console.log("token addresss is: ", token.address.toString());
      ...
      tokens.push(token); // 循环生成8种代币，存入 tokens数组中

      // console.log(account)  -- 0xc783df8a850f42e7F7e57013759C285caa701eB6
      // console.log(account2) -- 0xeAD9C93b79Ae7C1591b1FB5323BD777E86e150d4
    }

    smartpool = (await run("deploy-libraries-and-smartpool")) as Pv2SmartPool;
    // console.log(smartpool) -- 有很多内容

    await smartpool.init(pool.address, NAME, SYMBOL, INITIAL_SUPPLY); // smartpool 中数据的初始化
    
    // console.log("smart pool init:",pool.address, NAME, SYMBOL, INITIAL_SUPPLY.toString())
    // 0x7c2C195CD6D34B8F845992d380aADB2730bB9C6F   TEST POOL   TPL   1000000000000000000
    ...
    for (const token of tokens) {
      ...
      console.log("constants.MaxUint256:",constants.MaxUint256.toString())
    }

    startBlock = (await ethers.provider.getBlockNumber()) + 1;  // 
    endBlock = startBlock + 100; // 结束区块 = 开始区块 + 100
    console.log("block is:",startBlock,endBlock) // 66   166     132  232
  });
  ...
  describe("updateWeight()", async () => {
    it("Updating the weigth from a non controller should fail", async () => {
      ...
      // console.log("smartpool is",smartpool); -- smartpool 中有非常多的内容
    ...
    it("Updating down should work", async () => {
      ...
      // console.log("update:",weightBefore.toString(),newWeight.toString())  2000000000000000000 1000000000000000000
  ...
  describe("updateWeightsGradually()", async () => {
    ...
    console.log("updateWeightsGradually:",weightsFixtureUp.toString(),weightsFixtureTokenAboveMax.toString())
    ...
    it("Setting a start block in the past should set it to the current block", async () => {
      ...
      // console.log("currentWeights is :",currentWeights.toString()) -- 2000000000000000000
  ...
  describe("pokeWeight()", async () => {
    ...
    describe("Adding tokens", async () => {
      ...
      it("commitAddToken should work", async () => {
        ...
        const newTokenStruct = await smartpool.getNewToken(); // 新代币结构
